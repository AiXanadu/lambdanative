...
#ifdef DEBUG_MEM
#define DMSG(fmt...) (fprintf(stderr,"DEBUG: MEM: " fmt),fprintf(stderr,"\n"))
#else
#define DMSG(fmt...)
#endif
...
voidpf ZCALLBACK fopen_mem_func (opaque, filename, mode)
   ...
   int mode;
{
   DMSG("fopen_mem_func %p %s %i", opaque, filename, mode);
...
uLong ZCALLBACK fread_mem_func (opaque, stream, buf, size)
   ...
   uLong size;
{
   DMSG("fread_mem_func %p %p %p %lu", opaque, stream, buf, size);
...
uLong ZCALLBACK fwrite_mem_func (opaque, stream, buf, size)
   ...
   uLong size;
{
   DMSG("fwrite_mem_func %p %p %p %lu", opaque, stream, buf, size);
    ourmemory_t *mem = (ourmemory_t *)stream;
